...
use anyhow::Result;
use gloamy::providers::{ChatMessage, ChatRequest, Provider, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = gloamy::providers::create_provider_with_options("openai-codex", None, &opts)?;
