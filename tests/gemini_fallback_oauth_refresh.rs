...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.gloamy/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.gloamy/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let gloamy_dir = PathBuf::from(home).join(".gloamy");
    let auth_profiles_path = gloamy_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: gloamy auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: gloamy auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.gloamy/auth-profiles.json (with expired token)
    let provider = gloamy::providers::create_provider("gemini", None)?;
