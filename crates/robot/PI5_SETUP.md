...
Complete guide to setting up a Gloamy-powered robot on Raspberry Pi 5.
...
# Test
echo "Hello, I am your robot!" | piper --model ~/.gloamy/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
```toml
# ~/.gloamy/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.gloamy/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/gloamy_sensors.fifo"
...
```bash
# Configure Gloamy to use robot tools
cat > ~/.gloamy/config.toml << 'EOF'
...
# Start sensor loop
if [ ! -p /tmp/gloamy_sensors.fifo ]; then
    mkfifo /tmp/gloamy_sensors.fifo
fi
...
```bash
# /etc/systemd/system/gloamy-robot.service
sudo tee /etc/systemd/system/gloamy-robot.service << 'EOF'
[Unit]
Description=Gloamy Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/gloamy
...
sudo systemctl enable gloamy-robot
sudo systemctl start gloamy-robot

# Check status
sudo systemctl status gloamy-robot
journalctl -u gloamy-robot -f  # View logs
```
