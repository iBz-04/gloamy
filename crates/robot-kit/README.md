...
A complete toolkit for building AI-powered robots with Gloamy. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Gloamy with robot tools

```bash
# Clone and build
git clone https://github.com/iBz-04/gloamy
cd gloamy
cargo build -p gloamy-robot-kit --release
```
...
# Copy config
mkdir -p ~/.gloamy
cp crates/robot-kit/robot.toml ~/.gloamy/
cp crates/robot-kit/SOUL.md ~/.gloamy/workspace/
...
# Test in mock mode
./target/release/gloamy agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/gloamy agent -m "Move forward 1 meter"
```
...
```rust
use gloamy_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `gloamy`, add a thin adapter that maps this
...
mkdir gloamy-robot-kit
cp -r target/release/gloamy gloamy-robot-kit/
cp -r examples/robot_kit gloamy-robot-kit/
cp -r ~/.gloamy gloamy-robot-kit/dot-gloamy

# Include models
mkdir -p gloamy-robot-kit/models
cp ~/.gloamy/models/ggml-base.bin gloamy-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf gloamy-robot-kit.tar.gz gloamy-robot-kit/

# Copy to USB
cp gloamy-robot-kit.tar.gz /media/usb/TarBalls/
```
