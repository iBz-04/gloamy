...
const SERVICE_LABEL: &str = "com.gloamy.daemon";
const WINDOWS_TASK_NAME: &str = "Gloamy Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "gloamy.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["gloamy", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "gloamy.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["gloamy", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "gloamy.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["gloamy", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["gloamy", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/gloamy");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/gloamy");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "gloamy", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove gloamy from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/gloamy)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: gloamy service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=Gloamy daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "gloamy.service"]));
...
/// Check if the gloamy user exists and has expected properties.
...
fn check_gloamy_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "gloamy"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser gloamy && delgroup gloamy",
            "addgroup -S gloamy && adduser -S -s /sbin/nologin -H -D -G gloamy gloamy",
        )
    } else {
        ("userdel gloamy", "useradd -r -s /sbin/nologin gloamy")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'gloamy' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'gloamy' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/gloamy" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: gloamy user has home directory '{}' (expected /var/lib/gloamy or /nonexistent)",
                        home
                    );
                }
...
fn ensure_gloamy_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "gloamy"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_gloamy_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "gloamy"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "gloamy"])
                .output()
                .context("Failed to create gloamy group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create gloamy group: {}", stderr.trim());
            }
            println!("✅ Created system group: gloamy");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create gloamy user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create gloamy user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "gloamy"])
            .output()
            .context("Failed to create gloamy user")?;
    ...
    println!("✅ Created system user: gloamy");
    Ok(())
}

/// Change ownership of a path to gloamy:gloamy
#[cfg(unix)]
fn chown_to_gloamy(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["gloamy:gloamy", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to gloamy:gloamy: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_gloamy(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_gloamy(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "gloamy:gloamy", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to gloamy:gloamy: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_gloamy(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".gloamy"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'gloamy' cannot write {} ({details}). \
             Re-run `sudo gloamy service install` and ensure ownership is gloamy:gloamy.",
            path.display(),
        );
    }
...
command_user="gloamy:gloamy"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/gloamy/access.log"
error_log="/var/log/gloamy/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/gloamy");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo gloamy service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/gloamy");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/gloamy");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_gloamy(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by gloamy:gloamy)",
    ...
    chown_recursive_to_gloamy(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/gloamy");
    ...
    run_checked(Command::new("rc-update").args(["add", "gloamy", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/gloamy");
    println!("   Config path: /etc/gloamy/config.toml");
    println!("   Start with: sudo gloamy service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("gloamy-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/gloamy.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "Gloamy Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/gloamy");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/gloamy"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"gloamy\""));
        assert!(script.contains("description=\"Gloamy daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/gloamy\""));
        assert!(script.contains("command_args=\"--config-dir /etc/gloamy daemon\""));
        assert!(!script.contains("env GLOAMY_CONFIG_DIR"));
        assert!(!script.contains("env GLOAMY_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"gloamy:gloamy\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/gloamy/access.log\""));
        assert!(script.contains("error_log=\"/var/log/gloamy/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/gloamy");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/gloamy");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/gloamy");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/gloamy"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/gloamy/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/gloamy/workspace'".to_string(),
                "gloamy".to_string()
            ]
        );
    }
