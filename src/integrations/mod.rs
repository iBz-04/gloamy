...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `gloamy onboard --interactive` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: gloamy onboard --channels-only");
            println!("    4. Start: gloamy channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: gloamy onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    Gloamy can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.gloamy/workspace/cron/");
            println!("    Run: gloamy cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: gloamy gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/theonlyhennygod/gloamy");
            }
        }
    }
