...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn chat_with_system(
        ...
        let credential = self.credential.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "{} API key not set. Run `gloamy onboard` or set the appropriate env var.",
                self.name
            )
        })?;
...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are Gloamy".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn user_agent_constructor_keeps_native_tool_calling_enabled() {
        ...
        assert!(caps.native_tool_calling);
        assert!(!caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("gloamy-test/1.0"));
    }
    ...
    fn user_agent_and_vision_constructor_preserves_capability_flags() {
        ...
        assert!(caps.native_tool_calling);
        assert!(caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("gloamy-test/vision"));
    }
