...
// in `~/.gloamy/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.gloamy/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(gloamy_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: gloamy_dir.join(".secret_key"),
            enabled,
        }
    }
