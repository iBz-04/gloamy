...
Examples:
  gloamy agent                              # interactive session
  gloamy agent -m \"Summarize today's logs\"  # single message
  gloamy agent -p anthropic --model claude-sonnet-4-20250514
  gloamy agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  gloamy gateway                  # use config defaults
  gloamy gateway -p 8080          # listen on port 8080
  gloamy gateway --host 0.0.0.0   # bind to all interfaces
  gloamy gateway -p 0             # random available port")]
    Gateway {
...
Launches the full Gloamy runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run Gloamy in \
production or as an always-on assistant.

Use 'gloamy service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  gloamy daemon                   # use config defaults
  gloamy daemon -p 9090           # gateway on port 9090
  gloamy daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `gloamy estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `gloamy estop`.
        #[arg(long, value_enum)]
...
Examples:
  gloamy cron list
  gloamy cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  gloamy cron add '*/30 * * * *' 'Check system health'
  gloamy cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  gloamy cron add-every 60000 'Ping heartbeat'
  gloamy cron once 30m 'Run backup in 30 minutes'
  gloamy cron pause <task-id>
  gloamy cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect Gloamy \
...
Examples:
  ...
  gloamy channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  gloamy channel remove my-bot
  gloamy channel bind-telegram gloamy_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  gloamy hardware introspect /dev/ttyACM0
  gloamy hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: gloamy::HardwareCommands,
    },
...
Examples:
  ...
  gloamy peripheral add nucleo-f401re /dev/ttyACM0
  gloamy peripheral add rpi-gpio native
  gloamy peripheral flash --port /dev/cu.usbmodem12345
  gloamy peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: gloamy::PeripheralCommands,
    },
...
Examples:
  gloamy memory stats
  gloamy memory list
  gloamy memory list --category core --limit 10
  gloamy memory get <key>
  gloamy memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  gloamy config schema              # print JSON Schema to stdout
  gloamy config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `gloamy`.
...
Examples:
  ...
  gloamy completions fish > ~/.config/fish/completions/gloamy.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("GLOAMY_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("GLOAMY_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for Gloamy.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting Gloamy Gateway on {host} (random port)");
            } else {
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting Gloamy Daemon on {host} (random port)");
            } else {
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `gloamy auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `gloamy auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `gloamy auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `gloamy auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `gloamy auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `gloamy auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["gloamy", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["gloamy", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["gloamy", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["gloamy", "estop", "resume", "--domain", "*.chase.com"])
