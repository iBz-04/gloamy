...
//! Gloamy to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "gloamy.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "gloamy.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["gloamy", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["gloamy", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<RuntimeConfigReload> {
    ...
    if let Some(gloamy_dir) = path.parent() {
        let store = crate::security::SecretStore::new(gloamy_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `gloamy models refresh --provider {}`.",
    ...
    if native_tools {
        prompt.push_str(
            "## Your Task\n\n\
             ...
             1. **File-based generators for NEW documents (highest priority when creating from scratch):** For \"create a PowerPoint / Word / Excel / PDF\" tasks, prefer native file generators over app scripting: the `pptx` skill (python-pptx), the `docx` skill (python-docx), the `xlsx` skill (openpyxl), and the `pdf-manipulation` skill. These produce a file in seconds without opening any app. Only fall back to the `automating-<app>` skill when the task explicitly modifies an EXISTING document the user is already editing.\n\
             2. **Skills first for existing state (`automating-<app>`):** If the task is about an app the user already has open — reading notes, sending a Messages thread, toggling reminders, driving Chrome — use the matching `automating-<app>` skill (e.g. automating-notes, automating-reminders, automating-calendar, automating-contacts, automating-mail, automating-messages, automating-chrome, automating-excel, automating-word, automating-pages, automating-keynote, automating-numbers, automating-powerpoint, automating-voice-memos). Skills invoke JXA/osascript via the shell tool. NEVER skip a matching skill to go straight to mac_automation or perception_capture.\n\
             ...
             5. **Shell / file / memory tools:** For general-purpose operations.\n\
             6. **mac_automation (last resort for GUI):** Use ONLY when NO matching skill exists AND the task requires direct GUI interaction. mac_automation is slow and fragile — treat it as a last resort. The runtime will refuse `mac_automation` (including `run_applescript`) whenever a matching `automating-<app>` skill is loaded; do not try to bypass this.\n\
        ...
        prompt.push_str(
            "## Your Task\n\n\
             ...
             Do not invent tool or action names. Do not refuse tool-capable requests without attempting the relevant tool first.\n\n\
             ## Tool Selection Priority (MANDATORY — follow this order)\n\n\
             1. **File-based generators for NEW documents:** For \"create a PowerPoint / Word / Excel / PDF\" tasks, prefer `pptx`, `docx`, `xlsx`, and `pdf-manipulation` skills over UI scripting. They generate files in seconds without launching any app.\n\
             2. **Skills for existing state (`automating-<app>`):** For driving an already-open app (Notes, Reminders, Calendar, Contacts, Mail, Messages, Chrome, Excel, Word, Pages, Keynote, Numbers, PowerPoint, Voice Memos). If a matching skill is loaded, use its scripting approach (JXA/osascript via the shell tool) FIRST. NEVER skip a matching skill to go straight to mac_automation or perception_capture.\n\
             3. **One CLI (preferred for third-party services):** When you need to interact with external services (Gmail, Slack, GitHub, Google Drive, etc.) and the `one` tool is available, use it. One CLI is the preferred integration tool.\n\
             4. **Composio (fallback for third-party services):** Use `composio` only when: (a) the `one` tool is not available, OR (b) the specific action is not available in One CLI, OR (c) One CLI fails for the specific action. Do not use Composio when One CLI can do the same thing.\n\
             5. **Shell / file / memory tools:** For general-purpose operations.\n\
             6. **mac_automation (last resort for GUI):** Use ONLY when NO matching skill exists AND the task requires direct GUI interaction. mac_automation is slow and fragile — treat it as a last resort. The runtime will refuse `mac_automation` (including `run_applescript`) whenever a matching `automating-<app>` skill is loaded; do not try to bypass this.\n\
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `gloamy onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `gloamy daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `gloamy service stop && gloamy service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        if !listed.contains("com.gloamy.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.gloamy.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.gloamy.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/gloamy");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: gloamy channel start");
            println!("To check health:    gloamy channel doctor");
            println!("To configure:      gloamy onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `gloamy onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.gloamy/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(MatrixChannel::new_with_session_hint_and_gloamy_dir(
                mx.homeserver.clone(),
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `gloamy onboard` first.");
        return Ok(());
    }

    println!("🩺 Gloamy Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `gloamy gateway` then GET /health");
    }
...
async fn start_channels_internal(config: Config, run_scheduler: bool) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `gloamy onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Gloamy Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                gloamy_dir: Some(temp.path().to_path_buf()),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Gloamy 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "gloamy.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "gloamy.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["gloamy", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["gloamy", "restart"]);
    }
