//! Flash Gloamy Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Gloamy Nucleo firmware. Builds from firmware/gloamy-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (gloamy's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("gloamy-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from gloamy repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Gloamy Nucleo firmware...");
    ...
    println!("Gloamy Nucleo firmware flashed successfully.");
