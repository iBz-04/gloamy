    ...
    fn build(&self, _ctx: &PromptContext<'_>) -> Result<String> {
        Ok(
            "## Execution Loop\n\n\
             ...
             This ladder applies to PUBLIC web information only. For data that lives in a connected account (the user's email, posts, calendar, files, etc.), use the `one` tool instead — do NOT fall back to browser-scraping authenticated sites; if One CLI fails, report its error and the smallest fix needed.\n\
             When asked to find public information from the web, follow this ladder in order. Move to the next rung ONLY if the current one fails:\n\
             ...
             3. **One CLI (preferred for third-party services):** When you need to interact with external services (Gmail, Slack, GitHub, Google Drive, etc.) and the `one` tool is available, use it. One CLI is the preferred integration tool. Before execute_action, ALWAYS call get_action_knowledge for the chosen action to learn its required path_vars/query_params/data — do not guess parameters. Path variable names must exactly match that action's {{placeholders}} (casing varies per action). If execute_action fails, read the returned validation error and fix the named parameters instead of switching tools. The One CLI is reachable ONLY through the `one` tool — never probe or invoke the `one` binary via the shell tool (the security policy will reject it).\n\
