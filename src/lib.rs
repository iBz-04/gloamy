...
Examples:
  ...
  gloamy channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `Gloamy` workspace
    Openclaw {
...
Examples:
  ...
  gloamy cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  gloamy cron add-every 60000 'Ping heartbeat'     # every minute
  gloamy cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  gloamy cron once 2h 'Follow up on deployment'
  gloamy cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  gloamy cron update <task-id> --expression '0 8 * * *'
  gloamy cron update <task-id> --tz Europe/London --name 'Morning check'
  gloamy cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  gloamy hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  gloamy peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash Gloamy firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash Gloamy firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  gloamy peripheral flash
  gloamy peripheral flash --port /dev/cu.usbmodem12345
  gloamy peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash Gloamy firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
