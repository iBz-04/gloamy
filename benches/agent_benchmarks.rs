//! Performance benchmarks for Gloamy hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/iBz-04/gloamy/issues/618 (item 7)
...
use gloamy::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use gloamy::config::MemoryConfig;
use gloamy::memory;
use gloamy::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "gloamy architecture"}}
</tool_call>
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about gloamy agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("gloamy agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
