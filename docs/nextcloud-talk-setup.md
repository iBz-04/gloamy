...
This guide covers native Nextcloud Talk integration for Gloamy.
...
Add this section in `~/.gloamy/config.toml`:
...
gloamy daemon
# or
gloamy gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, Gloamy verifies:
...
- Gloamy ignores bot-originated webhook events (`actorType = bots`).
- Gloamy ignores non-message/system events.
...
3. Confirm Gloamy receives and replies in the same room.
