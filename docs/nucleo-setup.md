# Gloamy on Nucleo-F401RE — Step-by-Step Guide

Run Gloamy on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

Gloamy can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
Gloamy includes everything for Nucleo-F401RE:
...
| Flash command | `gloamy peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via Gloamy

From the gloamy repo root:

```bash
gloamy peripheral flash-nucleo
```

This builds `firmware/gloamy-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `gloamy daemon` or `gloamy agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `gloamy peripheral list` to verify.
