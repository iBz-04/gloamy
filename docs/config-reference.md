# Gloamy Config Reference (Operator-Oriented)
...
1. `GLOAMY_WORKSPACE` override (if set)
2. persisted `~/.gloamy/active_workspace.toml` marker (if present)
3. default `~/.gloamy/config.toml`

Gloamy logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `gloamy config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `gloamy` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `gloamy doctor traces --limit 20`
  - `gloamy doctor traces --event tool_call_result --contains \"error\"`
  - `gloamy doctor traces --id <trace-id>`
...
- When `enabled = true` and no OTP secret exists, Gloamy generates one and prints an enrollment URI once.
...
| `state_file` | `~/.gloamy/estop-state.json` | Persistent estop state path |
...
- Use CLI command `gloamy estop` to engage and `gloamy estop resume` to clear levels.
...
- Security-first default: Gloamy does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `GLOAMY_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `GLOAMY_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `GLOAMY_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
...
- Gloamy requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
- `allowed_commands` is hot-reloadable at runtime through the security policy, so config reloads can tighten or widen shell access without restarting the daemon.
...
3. Validate with `gloamy doctor` before restart/rollout.
...
- `GLOAMY_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `GLOAMY_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
