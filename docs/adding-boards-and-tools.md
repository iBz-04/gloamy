# Adding Boards and Tools — Gloamy Hardware Guide

This guide explains how to add new hardware boards and custom tools to Gloamy.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.gloamy/config.toml)
gloamy peripheral add nucleo-f401re /dev/ttyACM0
gloamy peripheral add arduino-uno /dev/cu.usbmodem12345
gloamy peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Gloamy can index PDF files:
...
2. **Add to config** — `gloamy peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `gloamy peripheral list` | List configured boards |
| `gloamy peripheral add <board> <path>` | Add board (writes config) |
| `gloamy peripheral flash` | Flash Arduino firmware |
| `gloamy peripheral flash-nucleo` | Flash Nucleo firmware |
| `gloamy hardware discover` | List USB devices |
| `gloamy hardware info` | Chip info via probe-rs |
