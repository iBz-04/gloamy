...
Gloamy provides prebuilt binaries for Android devices.
...
The easiest way to run Gloamy on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/iBz-04/gloamy/releases/latest/download/gloamy-aarch64-linux-android.tar.gz
tar xzf gloamy-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/iBz-04/gloamy/releases/latest/download/gloamy-armv7-linux-androideabi.tar.gz
tar xzf gloamy-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run Gloamy outside Termux:

```bash
# From your computer with ADB
adb push gloamy /data/local/tmp/
adb shell chmod +x /data/local/tmp/gloamy
adb shell /data/local/tmp/gloamy --version
```
