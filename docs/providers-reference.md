...
3. Generic fallback env vars: `GLOAMY_API_KEY` then `API_KEY`
...
- After multimodal normalization, Gloamy sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, Gloamy returns a structured capability error instead of silently ignoring images.
...
- Gloamy normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `gloamy models refresh --provider llamacpp`
...
- Model discovery: `gloamy models refresh --provider sglang`
...
- Model discovery: `gloamy models refresh --provider vllm`
...
- Model discovery: `gloamy models refresh --provider osaurus`
...
- Model discovery: `gloamy models refresh --provider nvidia`
