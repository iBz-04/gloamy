...
- `cargo check` / `cargo build` dừng lâu ở `Checking gloamy`
...
### Không tìm thấy lệnh `gloamy` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `gloamy`
...
- Dừng các tiến trình `gloamy daemon` / `gloamy channel start` thừa
