...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong Gloamy.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `gloamy daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.gloamy/config.toml`.
...
Khi chạy `gloamy channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `gloamy models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

Gloamy hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `gloamy channel list`, `gloamy channel doctor`, và `gloamy channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@gloamy:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
Gloamy hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `gloamy daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/gloamy.log
```
