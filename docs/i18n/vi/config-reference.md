...
1. Biến `GLOAMY_WORKSPACE` (nếu được đặt)
2. Marker `~/.gloamy/active_workspace.toml` (nếu có)
3. Mặc định `~/.gloamy/config.toml`

Gloamy ghi log đường dẫn config đã giải quyết khi khởi động ở mức `INFO`:

- `Config loaded` với các trường: `path`, `workspace`, `source`, `initialized`

Lệnh xuất schema:

- `gloamy config schema` (xuất JSON Schema draft 2020-12 ra stdout)
...
| `otel_service_name` | `gloamy` | Tên dịch vụ gửi đến OTLP collector |
...
- Mặc định an toàn: Gloamy **không** clone hay đồng bộ `open-skills` trừ khi `open_skills_enabled = true`.
- Ghi đè qua biến môi trường:
  - `GLOAMY_OPEN_SKILLS_ENABLED` chấp nhận `1/0`, `true/false`, `yes/no`, `on/off`.
  - `GLOAMY_OPEN_SKILLS_DIR` ghi đè đường dẫn kho khi có giá trị.
- Thứ tự ưu tiên: `GLOAMY_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` trong `config.toml` → mặc định `false`.
...
- Gloamy yêu cầu Composio v3 tools với `toolkit_versions=latest` và thực thi với `version="latest"` để tránh bản tool mặc định cũ.
...
3. Kiểm tra bằng `gloamy doctor` trước khi khởi động lại/triển khai.
...
- Khi `gloamy channel start` đang chạy, thay đổi `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url` và `reliability.*` được áp dụng nóng từ `config.toml` ở tin nhắn tiếp theo.
