...
[INFO] ✓ Audit logging enabled (~/.config/gloamy/audit.log)
...
$ gloamy security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/gloamy/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ gloamy config set security.sandbox.enabled false

# Bật backend cụ thể
$ gloamy config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ gloamy config set security.resources.max_memory_mb 2048
```
...
```bash
$ gloamy onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `gloamy security --status`
