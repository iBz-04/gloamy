# Gloamy Security Improvement Roadmap
...
Gloamy already has **excellent application-layer security**:
...
## Comparison: Gloamy vs PicoClaw vs Production Grade

| Feature | PicoClaw | Gloamy Now | Gloamy + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`gloamy audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/gloamy/audit.log"
sign_events = true
max_size_mb = 100
...
gloamy security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
gloamy audit --user @alice --since 24h
gloamy audit --risk high --violations-only
gloamy audit --verify-signatures
...
**Gloamy is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Gloamy becomes:
...
**Value**: Transforms Gloamy from "safe for testing" to "safe for production"
