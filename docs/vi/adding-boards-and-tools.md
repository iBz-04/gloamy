# Thêm Board và Tool — Hướng dẫn phần cứng Gloamy

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào Gloamy.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.gloamy/config.toml)
gloamy peripheral add nucleo-f401re /dev/ttyACM0
gloamy peripheral add arduino-uno /dev/cu.usbmodem12345
gloamy peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, Gloamy có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `gloamy peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `gloamy peripheral list` | Liệt kê các board đã cấu hình |
| `gloamy peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `gloamy peripheral flash` | Nạp firmware Arduino |
| `gloamy peripheral flash-nucleo` | Nạp firmware Nucleo |
| `gloamy hardware discover` | Liệt kê thiết bị USB |
| `gloamy hardware info` | Thông tin chip qua probe-rs |
