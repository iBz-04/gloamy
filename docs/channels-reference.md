...
This document is the canonical reference for channel configuration in Gloamy.
...
6. **Stale runtime state**: config changed but `gloamy daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.gloamy/config.toml`.
...
When running `gloamy channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `gloamy models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

Gloamy supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `gloamy channel list`, `gloamy channel doctor`, and `gloamy channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@gloamy:matrix.example.com"   # optional, recommended for E2EE
...
- `GLOAMY_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
5. Restart `gloamy daemon` after config changes.
...
```bash
RUST_LOG=info gloamy daemon 2>&1 | tee /tmp/gloamy.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/gloamy.log
```
