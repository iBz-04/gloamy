...
usage() {
  cat <<'USAGE'
Gloamy installer bootstrap engine

Usage:
  ./gloamy_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds Gloamy only (requires existing Rust toolchain).
...
Examples:
  ...
  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/iBz-04/gloamy/master/scripts/bootstrap.sh | bash

Environment:
  GLOAMY_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  ...
  GLOAMY_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${GLOAMY_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${GLOAMY_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/iBz-04/gloamy/releases/latest/download/gloamy-${target}.tar.gz"
  temp_dir="$(mktemp -d -t gloamy-prebuilt-XXXXXX)"
  archive_path="$temp_dir/gloamy-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/gloamy"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name gloamy -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable gloamy binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/gloamy"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/gloamy"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/gloamy-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  if prompt_yes_no "Install gloamy into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${GLOAMY_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set GLOAMY_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set GLOAMY_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${GLOAMY_DOCKER_IMAGE:-gloamy-bootstrap:local}"
  fallback_image="ghcr.io/iBz-04/gloamy:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.gloamy-docker"
  else
    default_data_dir="$WORK_DIR/.gloamy-docker"
  fi
  docker_data_dir="${GLOAMY_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.gloamy" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official Gloamy image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.gloamy:/gloamy-data/.gloamy"
  workspace_mount="$docker_data_dir/workspace:/gloamy-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  GLOAMY_API_KEY="sk-..." ./gloamy_install.sh --docker
or run interactive:
...
MSG
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/gloamy-data \
    -e GLOAMY_WORKSPACE=/gloamy-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/iBz-04/gloamy.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${GLOAMY_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set GLOAMY_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t gloamy-bootstrap-XXXXXX)"
...
Your containerized Gloamy data is persisted under:
DONE
...
or re-run with:
  ./gloamy_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing gloamy to cargo bin"
...
GLOAMY_BIN=""
if have_cmd gloamy; then
  GLOAMY_BIN="gloamy"
elif [[ -x "$HOME/.cargo/bin/gloamy" ]]; then
  GLOAMY_BIN="$HOME/.cargo/bin/gloamy"
elif [[ -x "$WORK_DIR/target/release/gloamy" ]]; then
  GLOAMY_BIN="$WORK_DIR/target/release/gloamy"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$GLOAMY_BIN" ]]; then
    error "onboarding requested but gloamy binary is not available."
    error "Run without --skip-install, or ensure gloamy is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$GLOAMY_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  GLOAMY_API_KEY="sk-..." ./gloamy_install.sh --onboard
or run interactive:
  ./gloamy_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$GLOAMY_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
