...
// The daemon owns this file on its own host; a browser cannot open it, so the
// path is surfaced for the operator to open in their editor of choice.
const CONFIG_PATH = '~/.gloamy/config.toml'
...
async function copyConfigPath() {
  try {
    ...
    setTimeout(() => { configPathCopied.value = false }, 1600)
  }
  catch (error) {
    console.error('Failed to copy config path:', error)
  }
}

function openWebsite() {
  window.open('https://www.gloamy.co', '_blank', 'noopener,noreferrer')
}
        ...
        <div class="flex items-center justify-between py-2 px-1">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-[12px] text-muted-foreground mt-0.5">Edit <code class="font-mono">{{ CONFIG_PATH }}</code> on the machine running the daemon</p>
            </div>
          </div>
          <button
          ...
          >
            {{ configPathCopied ? 'Copied' : 'Copy path' }}
            <Icon :icon="configPathCopied ? 'hugeicons:tick-02' : 'hugeicons:copy-01'" class="size-[13px]" />
          </button>
        </div>
      </section>
