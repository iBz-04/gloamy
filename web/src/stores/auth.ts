...
import { WebStore } from '@/lib/storage'

const store = new WebStore('auth.json')

const TOKEN_KEY = 'token'
const BASE_URL_KEY = 'api_base_url'
// Empty string means same-origin: requests go to /api and /pair on the page's
// own host, which the dev server (and any reverse proxy in front of a hosted
// build) forwards to the daemon. Set VITE_GLOAMY_API_BASE to talk to a daemon
// on another origin -- that origin must then allow this one via CORS.
