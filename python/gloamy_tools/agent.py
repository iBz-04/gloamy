...
SYSTEM_PROMPT = """You are Gloamy, an AI assistant with tool access. Use tools to accomplish tasks.
...
class GloamyAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "GloamyAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await GloamyAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a Gloamy agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured GloamyAgent instance
