...
Python companion package for [Gloamy](https://github.com/iBz-04/gloamy) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install gloamy-tools[discord]
```
...
# Run the CLI
gloamy-tools "List files in the current directory"
...
| Feature | Rust Gloamy | gloamy-tools |
|---------|---------------|----------------|
...
Use **Rust Gloamy** for production edge deployments. Use **gloamy-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
