# Gloamy Development Environment

A fully containerized development sandbox for Gloamy agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`gloamy-dev`)
...
- **Agent**: `/gloamy-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.gloamy` (mounted to `/gloamy-data/.gloamy`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.gloamy` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `gloamy-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=gloamy-target`) to speed repeat local image builds.
